  ...
  )
{
  ...
  if ((Urb->Data != NULL) && (Urb->DataMap == NULL)) {
    ...
    Len = Urb->DataLen;
    Status  = DmaMap (MapOp, Urb->Data, &Len, &PhyAddr, &Map);
  ...
  )
{
  ...
  //
  DcbaaPhy = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Dcbaa, Entries);
  ...
  CmdRingPhy = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, (VOID *)(UINTN) CmdRing, sizeof (TRB_TEMPLATE) * CMD_RING_TRB_NUMBER);
  ...
  )
{
  ...
  DequeuePhy = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Buf, Size);
  ...
  ERSTPhy = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, ERSTBase, Size);
  ...
  )
{
  ...
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Buf, sizeof (TRB_TEMPLATE) * TrbNum);
  ...
  )
{
  ...
  for (Index = 0; Index < Urb->TrbNum; Index++) {
    ...
    if (CheckedTrb->Type == TRB_TYPE_LINK) {
      ...
      CheckedTrb = (TRB_TEMPLATE *)(UINTN) UsbHcGetHostAddrForBusAddr (Xhc->MemPool, (VOID *)(UINTN) PhyAddr, sizeof (TRB_TEMPLATE));
  ...
  )
{
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    //
    // Need convert device address to host address
    //
    PhyAddr = (EFI_PHYSICAL_ADDRESS)(EvtTrb->TRBPtrLo | LShiftU64 ((UINT64) EvtTrb->TRBPtrHi, 32));
    TRBPtr = (TRB_TEMPLATE *)(UINTN) UsbHcGetHostAddrForBusAddr (Xhc->MemPool, (VOID *)(UINTN) PhyAddr, sizeof (TRB_TEMPLATE));
...
EXIT:
  ...
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Xhc->EventRing.EventRingDequeue, sizeof (TRB_TEMPLATE));
...
/**
  Flush data from controller specific address to mapped system
  memory address.
  ...
  )
{
  ...
  //
  PhyAddr = UsbHcGetBusAddrForHostAddr (
              Xhc->MemPool,
  ...
  //
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, OutputContext, sizeof (DEVICE_CONTEXT));
  //
  // Fill DCBAA with device address
  //
  ...
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Xhc->UsbDevContext[SlotId].InputContext, sizeof (INPUT_CONTEXT));
  ...
  )
{
  ...
  //    a pointer to the Output Device Context data structure (6.2.1).
  //
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, OutputContext, sizeof (DEVICE_CONTEXT_64));
  //
  // Fill DCBAA with device address
  //
  Xhc->DCBAA[SlotId] = (UINT64) (UINTN) PhyAddr;
  ...
  gBS->Stall (XHC_RESET_RECOVERY_DELAY);
  ZeroMem (&CmdTrbAddr, sizeof (CmdTrbAddr));
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Xhc->UsbDevContext[SlotId].InputContext, sizeof (INPUT_CONTEXT_64));
  CmdTrbAddr.PtrLo    = XHC_LOW_32BIT (PhyAddr);
  CmdTrbAddr.PtrHi    = XHC_HIGH_32BIT (PhyAddr);
  CmdTrbAddr.CycleBit = 1;
  CmdTrbAddr.Type     = TRB_TYPE_ADDRESS_DEV;
  ...
  )
{
  ...
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, InputContext, sizeof (INPUT_CONTEXT));
  ...
  )
{
  ...
  //
  ZeroMem (&CmdTrbCfgEP, sizeof (CmdTrbCfgEP));
  PhyAddr  = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, InputContext, sizeof (INPUT_CONTEXT_64));
  CmdTrbCfgEP.PtrLo    = XHC_LOW_32BIT (PhyAddr);
  CmdTrbCfgEP.PtrHi    = XHC_HIGH_32BIT (PhyAddr);
  CmdTrbCfgEP.CycleBit = 1;
  CmdTrbCfgEP.Type     = TRB_TYPE_CON_ENDPOINT;
  ...
  )
{
  ...
  PhyAddr = UsbHcGetBusAddrForHostAddr (Xhc->MemPool, Urb->Ring->RingEnqueue, sizeof (CMD_SET_TR_DEQ_POINTER));

  ...
  )
{
  ...
  Instance->AhciBaseAddress       = IdeControllerInit->AhciBaseAddress;
  ...
  )
{
  ...
  ClassCode = PCI_CLASS_MASS_STORAGE_SATADPA;
  switch (ClassCode) {
  ...
  )
{
  ...
  switch (Mode) {
    case EfiAtaAhciMode:
      ...
      Status = AhciPacketCommandExecute (Instance->AhciBaseAddress, &Instance->AhciRegisters, Port, PortMultiplier, Packet);
      break;
    default :
      Status = EFI_DEVICE_ERROR;
      break;
